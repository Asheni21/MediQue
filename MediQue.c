...
// ── 2. Linear Search — by ID or Name or Disease ──────────────────────────────
// Scans the array from index 0 to patientCount-1 (O(n))
void searchPatient() {
    int option;
...
// ── 5. Delete Patient (shift elements left) ───────────────────────────────────
void deletePatient() {
    int id; printf("\n  Enter ID to delete: "); scanf("%d", &id); getchar();
    for (int i = 0; i < patientCount; i++)
        if (patients[i].id == id) {
            // Shift all elements after index i one position left
...
// ── 7. Bubble Sort — by Age (youngest to oldest) ─────────────────────────────
...
// ── 8. Selection Sort — by ID ─────────────────────────────────────────────────
void sortByIDSelection() {
    if (patientCount == 0) { printf("\n  No records to sort.\n"); return; }
    Patient temp[MAX_PATIENTS];
    for (int i = 0; i < patientCount; i++) temp[i] = patients[i];
    // Selection sort on id
    for (int i = 0; i < patientCount - 1; i++) {
        int minIdx = i;
...
// ── 12. Display in Reverse Order (traverse array backwards) ──────────────────
void displayReverse() {
    if (patientCount == 0) { printf("\n  No records available.\n"); return; }
    printf("\n  ==========================================\n");
